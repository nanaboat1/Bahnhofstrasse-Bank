...
bool search_vault(std::vector<std::string> &search_client_data, std::stack<Client> &user)
{
  ...
  // Case :: TO prevent memory errors, the size of the stack get's checked befofe any 
  // further evaluation is made to the stack.
  if ( !(user.size() == 0) ) {

    // Case: When there is data but then it doesn't correspond to the user's input
...
void login_bank()
{
  ...
  // Used to exit the user input
  bool exit = false;
  bool accnt__found = false;
  ...
  // Case : Search the Vault for User details and if gotten, create a client object and load into
  // stack.
  accnt__found = search_vault(user_login_info, user);

  //clear_disp();

  // Case: If account not found, the programme should call login function again.
  if (accnt__found)
  {
    ...
    // CASE: INFORM THE USER OF CURRENT BALANCE.
    std::cout << " -------------------------------------------" << std::endl
              << std::endl;
    std::cout << " YOUR CURRENT BALANCE IS ==> [ " << user.top().get_cash_resrv() << " ]" << std::endl
              << std::endl;
    std::cout << " -------------------------------------------" << std::endl
              << std::endl;
    ...
    // Manipulates Basic Transactions.
    do
    {
      ...
      std::cout << " Press 1 to contiune " << std::endl;
      std::cin >> delay;
  ...
  } else { // Case: when account is not found.

    int restart_login =0;
    std::cout << " Try Logging in again " << std::endl;
    std::cout << " Press 1 to open logi function again or any number to quit login ";
    std::cin >> restart_login;


   // clears display 
    clear_disp();
    switch(restart_login) {
