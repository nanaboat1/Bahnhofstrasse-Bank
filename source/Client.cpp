...
// // gettors of the class
    ...
    bool Client::set_access( int arb_pin ) { 

        if ( arb_pin == std::stoi(pin)) {
            this->access = true;
        } else {
            this->access = false;
        }
...
// //  Constructors
    ...
    Client::Client( Biodata _bio, std::string _accntNum, std::string _pin ) :
        bio(_bio), accnt_num(_accntNum), pin(_pin) {  cash_resrv = 700.0; } // bonus money for creating acct.
...
// 
    ...
    void Client::update_cash_reserve( float update_cash ) {
        ...
        if ( Client::access && (this->cash_resrv >= update_cash )) {
