...
// // gettors of the class
    ...
    void Client::set_access( std::string arb_pin ) { 

        if ( std::stoi(this->get_pin()) == std::stoi(arb_pin) ) {
            this->access = true;
        }
...
// //  Constructors
    ...
    Client::Client( Biodata _bio, std::string _accntNum, std::string _pin ) :
        bio(_bio), accnt_num(_accntNum), pin(_pin) {  cash_resrv = 700.00; } // bonus money for creating acct.
...
// 
    ...
    void Client::incrs_cash_reserve( float update_cash ) {
    ...
    void Client::dcrs_cash_reserve( float update_cash ) {
...
// 
    ...
    float Client::operator-( float _amnt ) {
        ...
        if ( this->access && ( this->cash_resrv > _amnt )) {
            this->cash_resrv -= _amnt;
